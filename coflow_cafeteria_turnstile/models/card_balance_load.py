...
class CardBalanceLoad(models.Model):
    ...
    _description = 'Kart Bakiye Yüklemesi'
    ...
    load_date = fields.Datetime(
        string='Yükleme Tarihi',
        required=True,
        default=fields.Datetime.now,
        tracking=True,
    )
    amount = fields.Float(
        string='Yüklenen Tutar',
        required=True,
        digits=(12, 2),
        tracking=True,
    )
    ...
    month_period = fields.Char(
        string='Dönem (YYYY-MM)',
        help='Aylık yüklemeler için dönem bilgisi.',
    )
    ...
    def _compute_display_name(self):
        for rec in self:
            if rec.card_id and rec.load_date:
                ...
                rec.display_name = _('Yeni Bakiye Yüklemesi')

    @api.depends('transaction_ids', 'transaction_ids.price')
    def _compute_used_amount(self):
        for rec in self:
            rec.used_amount = sum(rec.transaction_ids.mapped('price')) if rec.transaction_ids else 0.0
    ...
    def action_confirm(self):
        """Bakiye yüklemeyi onaylar ve kart bakiyesini günceller."""
        for rec in self:
            if rec.state != 'draft':
    ...
    def action_cancel(self):
        """Bakiye yüklemeyi iptal eder."""
        for rec in self:
            if rec.state == 'cancelled':
                raise ValidationError(_('Kayıt zaten iptal edilmiş!'))

            if rec.state == 'confirmed':
                if rec.transaction_ids:
                    raise ValidationError(
                        _('Bu bakiye yüklemesine ait işlemler bulunmaktadır. '
                          'İptal etmek için önce işlemleri kaldırmalısınız.')
                    )
                rec.card_id.balance -= rec.amount
    ...
    def action_set_to_draft(self):
        """İptal edilen kayıtları tekrar taslak durumuna çeker."""
        for rec in self:
            if rec.state != 'cancelled':
    ...
    def action_view_transactions(self):
        ...
        return {
            ...
            'context': {
                'default_balance_load_id': self.id,
