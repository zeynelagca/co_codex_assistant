...
class AIAnalysisHistory(models.Model):
    ...
    @api.depends('task_id.partner_id', 'ticket_id.partner_id', 'ticket_id.customer_id')
    def _compute_partner_id(self):
        for record in self:
            if record.task_id:
                record.partner_id = record.task_id.partner_id
            elif record.ticket_id:
                if hasattr(record.ticket_id, 'partner_id') and record.ticket_id.partner_id:
                    record.partner_id = record.ticket_id.partner_id
                else:
                    record.partner_id = getattr(record.ticket_id, 'customer_id', False)
            else:
