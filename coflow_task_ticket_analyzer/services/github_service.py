...
class GithubService(models.AbstractModel):
    ...
    def get_github_credentials(self, project=None, partner=None):
        ...
        if not repo_url:
            ...
            repo_url = self.env['ir.config_parameter'].sudo().get_param('coflow.github_repo_url')
            token = self.env['ir.config_parameter'].sudo().get_param('coflow.github_token')
    ...
    def create_and_push_code(self, task=None, ticket=None, code_suggestion=None, solution_description=None):
        ...
        try:
            ...
            branch_name = f"coflow/{record_type}-{record.id}-{self._sanitize_branch_name(record_name)}"
            ...
            # Create Pull Request
            pr_title = f"[CoFlow] {record_type.capitalize()} #{record.id}: {record_name}"
            pr_body = f"""## Automated PR by CoFlow
...
---
*This PR was automatically generated by CoFlow Task Ticket Analyzer*
"""
...
---
*Created automatically by CoFlow Task Ticket Analyzer*
